...
function setCountdown() {
    ...
    */
    ...
    $(mainClass).each(function () { //for each countdown instance
        $("select[name='category'],input[name='filter_date'],input[name='end_soon'],select[name='sort_item'],input[name='search']").change(function () {
            for (let i = 0; i < id_interval.length; i++) {
                clearInterval(id_interval[i]);
            }
        });
        ...
        let begin_date = $(this).attr('data-beginDate');
        ...
        //call main function
        dateReplace(extraClass, date, fixTime, initText, zeroPad, begin_date); //prevent delay for the first time
        id_interval.push(setInterval(dateReplace, 1000, extraClass, date, fixTime, initText, zeroPad, begin_date));
    });


    function dateReplace(extraClass, date, fixTime, initText, zeroPad, begin_date) {
        let dif = timeDistance(date, fixTime);
        begin_date = new Date(begin_date);
        let text = initText;
        let zeroPadArr = [];
        if (dif[0] < 0 || dif[1] < 0 || dif[2] < 0 || dif[3] < 0 || begin_date > new Date()) {
            //countdown ended
    ...
    function timeDistance(date, fixTime) {
        ...
        let OffsetLocation = 7; //timezone hcm +7
